...
void BoltzmannMC::evaluateNonConstantVariables(){
	...
	// initialize the birth positions (positions where the electrons are created at the beginning of the simulation)
	electronBirthPositions = electronPositions;

	// initialize the displacements of each electron
	electronDisplacements = Eigen::ArrayXXd::Zero(nElectrons, 3);
	electronDisplacementsWithPositionChange = Eigen::ArrayXXd::Zero(nElectrons, 3);

	previousSamplingPointElectronDisplacements = Eigen::ArrayXXd::Zero(nElectrons, 3);
	previousSamplingPointElectronDisplacementsWithPositionChange = Eigen::ArrayXXd::Zero(nElectrons, 3);
	electronDisplacementsSquare = Eigen::ArrayXXd::Zero(nElectrons, 3);
	electronDisplacementsWithPositionChangeSquare = Eigen::ArrayXXd::Zero(nElectrons, 3);

	DiffCoeff2methodBulk = Eigen::ArrayXXd::Zero(nElectrons, 3);
	DiffCoeff2methodFlux = Eigen::ArrayXXd::Zero(nElectrons, 3);
	positionS_1Flux = Eigen::ArrayXXd::Zero(nElectrons, 3);
	positionS_2Flux = Eigen::ArrayXXd::Zero(nElectrons, 3);
	positionS_1Bulk = Eigen::ArrayXXd::Zero(nElectrons, 3);
	positionS_2Bulk = Eigen::ArrayXXd::Zero(nElectrons, 3);

	previousElectrondisplacements = Eigen::ArrayXXd::Zero(nElectrons, 3);
	...
	squareDisplacementsXYZFlux = Eigen::ArrayXXd::Zero(nElectrons,3);
	squareDisplacementsXYZBulk = Eigen::ArrayXXd::Zero(nElectrons,3);
	squareDisplacementsZFlux = Eigen::ArrayXd::Zero(nElectrons);
	squareDisplacementsZBulk = Eigen::ArrayXd::Zero(nElectrons);
...
void BoltzmannMC::electronDynamicsUntilSynchronization(){
	...
	while ((electronsToBeAdvanced != false).any()){
		...
		previousSamplingPointElectronDisplacements = electronDisplacements;
		previousSamplingPointElectronDisplacementsWithPositionChange = electronDisplacementsWithPositionChange;
		// perform all operations that cannot be performed in parallel
		nonParallelCollisionTasks();
	}
	// update the displacements (with and without position change) to account when there is no ionization nor attachement
	electronDisplacements += electronPositions - electronBirthPositions;
	electronDisplacementsWithPositionChange += electronPositions - electronBirthPositions;
	electronBirthPositions = electronPositions;
	// for the calculation of the error assotiated to the flux and bulk velocity (1st method)
	electronDisplacementsSquare += (electronDisplacements - previousSamplingPointElectronDisplacements)*(electronDisplacements - previousSamplingPointElectronDisplacements);
	electronDisplacementsWithPositionChangeSquare += (electronDisplacementsWithPositionChange - previousSamplingPointElectronDisplacementsWithPositionChange)*(electronDisplacementsWithPositionChange - previousSamplingPointElectronDisplacementsWithPositionChange);
...
void BoltzmannMC::nonParallelCollisionTasks(){
	...
	for (auto attachID: attachElectronIDs){
		...
		electronDisplacements.row(attachID) += electronPositions.row(attachID)-electronBirthPositions.row(attachID);
		...
		// update displacements and birth position
		electronDisplacementsWithPositionChange.row(attachID) += electronPositions.row(attachID)-electronBirthPositions.row(attachID);
		electronBirthPositions.row(attachID) = electronPositions.row(attachID);	
	...
	while (nEjected > 0){
		...
		if (randSelecID < nElectrons){
			...
			electronDisplacements.row(randSelecID) += electronPositions.row(randSelecID)-electronBirthPositions.row(randSelecID);
			...
			// update displacements and birth position
			electronDisplacementsWithPositionChange.row(randSelecID) += electronPositions.row(randSelecID)-electronBirthPositions.row(randSelecID);
			electronBirthPositions.row(randSelecID) = electronPositions.row(randSelecID);
...
void BoltzmannMC::calculateMeanDataForSwarmParams(){
	...
	if(nIntegrationPoints == 1){
		positionS_2Flux = electronDisplacements;
		positionS_2Bulk = electronDisplacementsWithPositionChange;
		countsTau = 0;
	}	

	// Calculate the diffusion coefficient using the 1st and second method
	if (nIntegrationPoints > 1 && nIntegrationPoints % ConstTauSampling == 0 && (currentSamplingIndex - ConstTauSampling) >= 0 && steadyStateTime != Constant::NON_DEF){
		++countsTau;

		double Tau = samplingTimes[currentSamplingIndex] - samplingTimes[currentSamplingIndex - ConstTauSampling];
		TauMed += Tau;
		Eigen::Array3d effectiveDisplacementFlux;
		Eigen::Array3d effectiveDisplacementBulk;
		
		// 1st method FLUX
		for (int i = 0; i < nElectrons; i++){
			Eigen::Array3d Pos_med = (electronDisplacements - previousElectrondisplacements).row(i);
			z_medFlux[i] += (rotationMatrix * Pos_med.matrix())[2];
			effectiveDisplacementFlux = electronDisplacements.row(i) - previousElectrondisplacements.row(i);
			effectiveDisplacementFlux = rotationMatrix * effectiveDisplacementFlux.matrix();
			effectiveDisplacementZFlux[i].push_back(effectiveDisplacementFlux[2]); // saving the displacement along z to make the calculation of the logitudinal diffusion coefficient at the end of the simulation, when z_medFlux is known.
			Eigen::Array3d squareDisplacementsFlux = effectiveDisplacementFlux*effectiveDisplacementFlux;
			squareDisplacementsXYZFlux.row(i) += squareDisplacementsFlux;
			// for the calculation of the error
			SquareSquareDisplacementZFlux(i,0) += squareDisplacementsFlux[0]*squareDisplacementsFlux[0];
			SquareSquareDisplacementZFlux(i,1) += squareDisplacementsFlux[1]*squareDisplacementsFlux[1];
			
			previousElectrondisplacements.row(i) = electronDisplacements.row(i);
		}

		// 1st method BULK
		for (int i = 0; i < nElectrons; i++){
			Eigen::Array3d Pos_med = (electronDisplacementsWithPositionChange - previousElectrondisplacementsWithPositionChange).row(i);
			z_medBulk[i] += (rotationMatrix * Pos_med.matrix())[2];
			effectiveDisplacementBulk = electronDisplacementsWithPositionChange.row(i) - previousElectrondisplacementsWithPositionChange.row(i);
			effectiveDisplacementBulk = rotationMatrix * effectiveDisplacementBulk.matrix();
			effectiveDisplacementZBulk[i].push_back(effectiveDisplacementBulk[2]); // saving the displacement along z to make the calculation of the logitudinal diffusion coefficient at the end of the simulation, when z_medBulk is known.
			Eigen::Array3d squareDisplacementsBulk = effectiveDisplacementBulk*effectiveDisplacementBulk;
			squareDisplacementsXYZBulk.row(i) += squareDisplacementsBulk;
			// for the calculation of the error
			SquareSquareDisplacementZBulk(i,0) += squareDisplacementsBulk[0]*squareDisplacementsBulk[0];
			SquareSquareDisplacementZBulk(i,1) += squareDisplacementsBulk[1]*squareDisplacementsBulk[1];

			previousElectrondisplacementsWithPositionChange.row(i) = electronDisplacementsWithPositionChange.row(i);
		}

		// 2nd method
		if (countsTau == 1){
			positionS_1Flux = electronDisplacements;
			positionS_1Bulk = electronDisplacementsWithPositionChange;
		}

		if (countsTau > 1){
			// Bulk
			Eigen::ArrayXXd DeltaR1Bulk = electronDisplacementsWithPositionChange - positionS_1Bulk;
			Eigen::ArrayXXd DeltaR2Bulk = electronDisplacementsWithPositionChange - positionS_2Bulk;
			W_Bulk += positionS_1Bulk - positionS_2Bulk;
			DeltaR2MeanElectronsBulk += DeltaR2Bulk;
			DeltaR1MeanElectronsBulk += DeltaR1Bulk;

			// Flux
			Eigen::ArrayXXd DeltaR1Flux = electronDisplacements - positionS_1Flux;
			Eigen::ArrayXXd DeltaR2Flux = electronDisplacements - positionS_2Flux;
			DeltaR2MeanElectronsFlux += DeltaR2Flux;
			DeltaR1MeanElectronsFlux += DeltaR1Flux;
			
			for (int i = 0; i < nElectrons; i++){
				// Bulk velocity
				SquareMeanW_Bulk.row(i) += (positionS_1Bulk.row(i) - positionS_2Bulk.row(i))*(positionS_1Bulk.row(i) - positionS_2Bulk.row(i));
				// Bulk
				Eigen::Array3d DeltaPosBulk = DeltaR2Bulk.row(i)*DeltaR2Bulk.row(i) - DeltaR1Bulk.row(i)*DeltaR1Bulk.row(i);
				DeltaR2DeltaR2_DeltaR1DeltaR1Bulk.row(i) += DeltaPosBulk;
				// for the calculation of the error
				DeltaR2DeltaR2_DeltaR1DeltaR1SquareBulk.row(i) += DeltaPosBulk*DeltaPosBulk;

				// Flux
				Eigen::Array3d DeltaPosFlux = DeltaR2Flux.row(i)*DeltaR2Flux.row(i) - DeltaR1Flux.row(i)*DeltaR1Flux.row(i);
				DeltaR2DeltaR2_DeltaR1DeltaR1Flux.row(i) += DeltaPosFlux;
				// for the calculation of the error
				DeltaR2DeltaR2_DeltaR1DeltaR1SquareFlux.row(i) += DeltaPosFlux*DeltaPosFlux;
			}

			// for the calculation of the error
			DeltaR1DeltaR1Bulk += DeltaR1Bulk*DeltaR1Bulk;
			DeltaR2DeltaR2Bulk += DeltaR2Bulk*DeltaR2Bulk;

			DeltaR1DeltaR1Flux += DeltaR1Flux*DeltaR1Flux;
			DeltaR2DeltaR2Flux += DeltaR2Flux*DeltaR2Flux;

			positionS_2Bulk = positionS_1Bulk;
			positionS_1Bulk = electronDisplacementsWithPositionChange;
			positionS_2Flux = positionS_1Flux;
			positionS_1Flux = electronDisplacements;
		}
	}
...
void BoltzmannMC::checkSteadyState(){
	...
	if ((averageFirstPart >= averageSecondPart && relStdSecondPart < 0.01)  || totalCollisionCounter >= maxCollisionsBeforeSteadyState){
		...
		// once steady state is reached, electronDisplacements and electronBirthPositions need to be re-initialized
		electronBirthPositions = electronPositions;
		electronDisplacements = Eigen::ArrayXXd::Zero(nElectrons, 3);
		electronDisplacementsWithPositionChange = Eigen::ArrayXXd::Zero(nElectrons, 3);
		...
		// for 1st method
		previousElectrondisplacementsWithPositionChange = electronDisplacementsWithPositionChange;
		DiffCoeffErrorsBulk = Eigen::ArrayXXd::Zero(nElectrons, 2);
		SquareSquareDisplacementZBulk = Eigen::ArrayXXd::Zero(nElectrons, 3);
		effectiveDisplacementZBulk.resize(nElectrons);
		for(auto& vec : effectiveDisplacementZBulk) {
        	vec.clear();
    	}
    	z_medBulk = Eigen::ArrayXd::Zero(nElectrons);

    	previousElectrondisplacements = electronDisplacements;
    	DiffCoeffErrorsFlux = Eigen::ArrayXXd::Zero(nElectrons, 2);
		SquareSquareDisplacementZFlux = Eigen::ArrayXXd::Zero(nElectrons, 3);
    	effectiveDisplacementZFlux.resize(nElectrons);
		for(auto& vec : effectiveDisplacementZFlux) {
        	vec.clear();
    	}
		z_medFlux = Eigen::ArrayXd::Zero(nElectrons);

		// for 2nd method
		SquareMeanW_Bulk = Eigen::ArrayXXd::Zero(nElectrons, 3);
		W_Bulk = Eigen::ArrayXXd::Zero(nElectrons, 3);
		W_BulkError = Eigen::ArrayXXd::Zero(nElectrons, 3);

		DiffCoeff2methodBulk = Eigen::ArrayXXd::Zero(nElectrons, 3);
		DeltaR2MeanElectronsBulk = Eigen::ArrayXXd::Zero(nElectrons, 3);
		DeltaR1MeanElectronsBulk = Eigen::ArrayXXd::Zero(nElectrons, 3);
		DeltaR2DeltaR2_DeltaR1DeltaR1Bulk = Eigen::ArrayXXd::Zero(nElectrons, 3);
		DeltaR2DeltaR2_DeltaR1DeltaR1SquareBulk = Eigen::ArrayXXd::Zero(nElectrons, 3);
		DiffCoeff2methodErrorBulk = Eigen::ArrayXXd::Zero(nElectrons, 3);
		DeltaR1DeltaR1Bulk = Eigen::ArrayXXd::Zero(nElectrons, 3);
		DeltaR2DeltaR2Bulk = Eigen::ArrayXXd::Zero(nElectrons, 3);

		DiffCoeff2methodFlux = Eigen::ArrayXXd::Zero(nElectrons, 3);
		DeltaR2MeanElectronsFlux = Eigen::ArrayXXd::Zero(nElectrons, 3);
		DeltaR1MeanElectronsFlux = Eigen::ArrayXXd::Zero(nElectrons, 3);
		DeltaR2DeltaR2_DeltaR1DeltaR1Flux = Eigen::ArrayXXd::Zero(nElectrons, 3);
		DeltaR2DeltaR2_DeltaR1DeltaR1SquareFlux = Eigen::ArrayXXd::Zero(nElectrons, 3);
		DiffCoeff2methodErrorFlux = Eigen::ArrayXXd::Zero(nElectrons, 3);
		DeltaR1DeltaR1Flux = Eigen::ArrayXXd::Zero(nElectrons, 3);
		DeltaR2DeltaR2Flux = Eigen::ArrayXXd::Zero(nElectrons, 3);
...
void BoltzmannMC::evaluateSwarmParameters(){
	...
	for (int i = 0; i < nElectrons; i++){
		electronDisplacements.row(i) += electronPositions.row(i)-electronBirthPositions.row(i);
		electronDisplacementsWithPositionChange.row(i) += electronPositions.row(i) - electronBirthPositions.row(i);
	}


	// ------------------------------------- 1st method -------------------------------------

	// evaluate displacement velocity
	// With Position Change (bulk)
	electronDisplacementsWithPositionChangeSquare = electronDisplacementsWithPositionChangeSquare*nIntegrationPoints/totalIntegratedTime/totalIntegratedTime;
	averagedDisplacementVelocityWithPositionChange = electronDisplacementsWithPositionChange.colwise().mean()/totalIntegratedTime;
	// averagedDisplacementVelocityWithPositionChangeError = MathFunctions::standardDeviationColwise(electronDisplacementsWithPositionChange)/totalIntegratedTime/std::sqrt(nElectrons);
	averagedDisplacementVelocityWithPositionChangeError = ((electronDisplacementsWithPositionChangeSquare - (electronDisplacementsWithPositionChange/totalIntegratedTime)*(electronDisplacementsWithPositionChange/totalIntegratedTime)).sqrt()).colwise().mean()/sqrt(nIntegrationPoints)/sqrt(nElectrons);

	// Without Position Change (flux)
	electronDisplacementsSquare = electronDisplacementsSquare*nIntegrationPoints/totalIntegratedTime/totalIntegratedTime;
	averagedDisplacementVelocity = electronDisplacements.colwise().mean()/totalIntegratedTime;
	// averagedDisplacementVelocityError = MathFunctions::standardDeviationColwise(electronDisplacements)/totalIntegratedTime/std::sqrt(nElectrons);
	averagedDisplacementVelocityError = ((electronDisplacementsSquare - (electronDisplacements/totalIntegratedTime)*(electronDisplacements/totalIntegratedTime)).sqrt()).colwise().mean()/sqrt(nIntegrationPoints)/sqrt(nElectrons);

	// evaltuate transverse and longitudinal diffusion coefficients
	TauMed /= countsTau;
	
	// flux
	z_medFlux /= countsTau;
	for (int n = 0; n < nElectrons; n++){
		for (int i = 0; i < countsTau; i++){
			double squareDisplacementZ = (effectiveDisplacementZFlux[n][i] - z_medFlux[n])*(effectiveDisplacementZFlux[n][i] - z_medFlux[n]);
			squareDisplacementsZFlux[n] += squareDisplacementZ;
			SquareSquareDisplacementZFlux(n,2) += squareDisplacementZ*squareDisplacementZ;
		}
	}

	squareDisplacementsXYZFlux /= countsTau;
	squareDisplacementsZFlux /= countsTau;

	// for the calculation of the error of the diffusion coefficient using the 1st method
	Eigen::ArrayXXd MeanDiffCoeffSquareFlux = squareDisplacementsXYZFlux*squareDisplacementsXYZFlux;
	Eigen::ArrayXd MeanDiffCoeffSquareZFlux = squareDisplacementsZFlux*squareDisplacementsZFlux;
	for (int i = 0; i < nElectrons; i++){
		double SigmaDiffCoeffX = std::sqrt(SquareSquareDisplacementZFlux(i,0)/countsTau - MeanDiffCoeffSquareFlux(i,0));
		double SigmaDiffCoeffY = std::sqrt(SquareSquareDisplacementZFlux(i,1)/countsTau - MeanDiffCoeffSquareFlux(i,1));
		double SigmaDiffCoeffZ = std::sqrt(SquareSquareDisplacementZFlux(i,2)/countsTau - MeanDiffCoeffSquareZFlux[i]);
		DiffCoeffErrorsFlux(i,0) = 0.25*(SigmaDiffCoeffX + SigmaDiffCoeffY)/TauMed/std::sqrt(countsTau);
		DiffCoeffErrorsFlux(i,1) = 0.5*SigmaDiffCoeffZ/TauMed/std::sqrt(countsTau);
	}

	DTransvFlux1stmethod = 0.25*(squareDisplacementsXYZFlux.col(0) + squareDisplacementsXYZFlux.col(1)).mean()/TauMed;
	DLongFlux1stmethod = 0.5*squareDisplacementsZFlux.mean()/TauMed;
	DTransvFlux1stmethodError = DiffCoeffErrorsFlux.col(0).mean()/std::sqrt(nElectrons);
	DLongFlux1stmethodError = DiffCoeffErrorsFlux.col(1).mean()/std::sqrt(nElectrons);

	// bulk
	z_medBulk /= countsTau;
	for (int n = 0; n < nElectrons; n++){
		for (int i = 0; i < countsTau; i++){
			double squareDisplacementZ = (effectiveDisplacementZBulk[n][i] - z_medBulk[n])*(effectiveDisplacementZBulk[n][i] - z_medBulk[n]);
			squareDisplacementsZBulk[n] += squareDisplacementZ;
			SquareSquareDisplacementZBulk(n,2) += squareDisplacementZ*squareDisplacementZ;
		}
	}

	squareDisplacementsXYZBulk /= countsTau;
	squareDisplacementsZBulk /= countsTau;

	// for the calculation of the error in the diffusion coefficient using the 1st method
	Eigen::ArrayXXd MeanDiffCoeffSquareBulk = squareDisplacementsXYZBulk*squareDisplacementsXYZBulk;
	Eigen::ArrayXd MeanDiffCoeffSquareZBulk = squareDisplacementsZBulk*squareDisplacementsZBulk;
	for (int i = 0; i < nElectrons; i++){
		double SigmaDiffCoeffX = std::sqrt(SquareSquareDisplacementZBulk(i,0)/countsTau - MeanDiffCoeffSquareBulk(i,0));
		double SigmaDiffCoeffY = std::sqrt(SquareSquareDisplacementZBulk(i,1)/countsTau - MeanDiffCoeffSquareBulk(i,1));
		double SigmaDiffCoeffZ = std::sqrt(SquareSquareDisplacementZBulk(i,2)/countsTau - MeanDiffCoeffSquareZBulk[i]);
		DiffCoeffErrorsBulk(i,0) = 0.25*(SigmaDiffCoeffX + SigmaDiffCoeffY)/TauMed/std::sqrt(countsTau);
		DiffCoeffErrorsBulk(i,1) = 0.5*SigmaDiffCoeffZ/TauMed/std::sqrt(countsTau);
	}

	// Bulk doesn't give the right result!!!
	DTransvBulk1stmethod = 0.25*(squareDisplacementsXYZBulk.col(0) + squareDisplacementsXYZBulk.col(1)).mean()/TauMed;
	DLongBulk1stmethod = 0.5*squareDisplacementsZBulk.mean()/TauMed;
	DTransvBulk1stmethodError = DiffCoeffErrorsBulk.col(0).mean()/std::sqrt(nElectrons);
	DLongBulk1stmethodError = DiffCoeffErrorsBulk.col(1).mean()/std::sqrt(nElectrons);

	// ------------------------------------- 2nd method -------------------------------------
	
	// 2nd method bulk velocity
	W_Bulk = W_Bulk/(countsTau-1)/TauMed;
	SquareMeanW_Bulk = SquareMeanW_Bulk/TauMed/TauMed/(countsTau-1);

	for (int i = 0; i < nElectrons; i++){
		double SigmaW_BulkX = std::sqrt(SquareMeanW_Bulk(i,0) - W_Bulk(i,0)* W_Bulk(i,0));
		double SigmaW_BulkY = std::sqrt(SquareMeanW_Bulk(i,1) - W_Bulk(i,1)* W_Bulk(i,1));
		double SigmaW_BulkZ = std::sqrt(SquareMeanW_Bulk(i,2) - W_Bulk(i,2)* W_Bulk(i,2));
		W_BulkError(i,0) = SigmaW_BulkX/std::sqrt(countsTau-1);
		W_BulkError(i,1) = SigmaW_BulkY/std::sqrt(countsTau-1);
		W_BulkError(i,2) = SigmaW_BulkZ/std::sqrt(countsTau-1);
	}

	// 2nd method Bulk Diff Coeff
	DeltaR1MeanElectronsBulk /= (countsTau-1);
	DeltaR2MeanElectronsBulk /= (countsTau-1);
	DeltaR2DeltaR2_DeltaR1DeltaR1Bulk /= (countsTau-1);
	DeltaR2DeltaR2_DeltaR1DeltaR1SquareBulk /= (countsTau-1);
	DeltaR1DeltaR1Bulk /= (countsTau-1);
	DeltaR2DeltaR2Bulk /= (countsTau-1);
	for (int i = 0; i < nElectrons; i++){
		DiffCoeff2methodErrorBulk(i,0) = std::sqrt(DeltaR2DeltaR2_DeltaR1DeltaR1SquareBulk(i,0) - DeltaR2DeltaR2_DeltaR1DeltaR1Bulk(i,0)*DeltaR2DeltaR2_DeltaR1DeltaR1Bulk(i,0))/std::sqrt(countsTau-1);
		DiffCoeff2methodErrorBulk(i,1) = std::sqrt(DeltaR2DeltaR2_DeltaR1DeltaR1SquareBulk(i,1) - DeltaR2DeltaR2_DeltaR1DeltaR1Bulk(i,1)*DeltaR2DeltaR2_DeltaR1DeltaR1Bulk(i,1))/std::sqrt(countsTau-1);
		DiffCoeff2methodErrorBulk(i,2) = std::sqrt(DeltaR2DeltaR2_DeltaR1DeltaR1SquareBulk(i,2) - DeltaR2DeltaR2_DeltaR1DeltaR1Bulk(i,2)*DeltaR2DeltaR2_DeltaR1DeltaR1Bulk(i,2))/std::sqrt(countsTau-1);
	}
	DiffCoeff2methodErrorBulk += (DeltaR1DeltaR1Bulk - DeltaR1MeanElectronsBulk*DeltaR1MeanElectronsBulk + DeltaR2DeltaR2Bulk - DeltaR2MeanElectronsBulk*DeltaR2MeanElectronsBulk)/(countsTau-1);
	DiffCoeff2methodErrorBulk /= (2*TauMed); 
	
	DiffCoeff2methodBulk = 0.5*(DeltaR2DeltaR2_DeltaR1DeltaR1Bulk - DeltaR2MeanElectronsBulk*DeltaR2MeanElectronsBulk + DeltaR1MeanElectronsBulk*DeltaR1MeanElectronsBulk)/TauMed;
	Eigen::Array3d DiffCoefSumOverElectronsBulk = DiffCoeff2methodBulk.colwise().mean();
	
	DTransvBulk2ndmethod = 0.5*(DiffCoefSumOverElectronsBulk[0] + DiffCoefSumOverElectronsBulk[1]);
	DLongBulk2ndmethod = DiffCoefSumOverElectronsBulk[2];

	DTransvBulk2ndmethodError = (DiffCoeff2methodErrorBulk.col(0) + DiffCoeff2methodErrorBulk.col(1)).mean()/2.0/std::sqrt(nElectrons);
	DLongBulk2ndmethodError = (DiffCoeff2methodErrorBulk.col(2)).mean()/std::sqrt(nElectrons);

	
	// 2nd method Flux Diff Coeff
	DeltaR1MeanElectronsFlux /= (countsTau-1);
	DeltaR2MeanElectronsFlux /= (countsTau-1);
	DeltaR2DeltaR2_DeltaR1DeltaR1Flux /= (countsTau-1);
	DeltaR2DeltaR2_DeltaR1DeltaR1SquareFlux /= (countsTau-1);
	DeltaR1DeltaR1Flux /= (countsTau-1);
	DeltaR2DeltaR2Flux /= (countsTau-1);
	for (int i = 0; i < nElectrons; i++){
		DiffCoeff2methodErrorFlux(i,0) = std::sqrt(DeltaR2DeltaR2_DeltaR1DeltaR1SquareFlux(i,0) - DeltaR2DeltaR2_DeltaR1DeltaR1Flux(i,0)*DeltaR2DeltaR2_DeltaR1DeltaR1Flux(i,0))/std::sqrt(countsTau-1);
		DiffCoeff2methodErrorFlux(i,1) = std::sqrt(DeltaR2DeltaR2_DeltaR1DeltaR1SquareFlux(i,1) - DeltaR2DeltaR2_DeltaR1DeltaR1Flux(i,1)*DeltaR2DeltaR2_DeltaR1DeltaR1Flux(i,1))/std::sqrt(countsTau-1);
		DiffCoeff2methodErrorFlux(i,2) = std::sqrt(DeltaR2DeltaR2_DeltaR1DeltaR1SquareFlux(i,2) - DeltaR2DeltaR2_DeltaR1DeltaR1Flux(i,2)*DeltaR2DeltaR2_DeltaR1DeltaR1Flux(i,2))/std::sqrt(countsTau-1);
	}
	DiffCoeff2methodErrorFlux += (DeltaR1DeltaR1Flux - DeltaR1MeanElectronsFlux*DeltaR1MeanElectronsFlux + DeltaR2DeltaR2Flux - DeltaR2MeanElectronsFlux*DeltaR2MeanElectronsFlux)/(countsTau-1);
	DiffCoeff2methodErrorFlux /= (2*TauMed); 
	
	DiffCoeff2methodFlux = 0.5*(DeltaR2DeltaR2_DeltaR1DeltaR1Flux - DeltaR2MeanElectronsFlux*DeltaR2MeanElectronsFlux + DeltaR1MeanElectronsFlux*DeltaR1MeanElectronsFlux)/TauMed;
	Eigen::Array3d DiffCoefSumOverElectronsFlux = DiffCoeff2methodFlux.colwise().mean();

	DTransvFlux2ndmethod = 0.5*(DiffCoefSumOverElectronsFlux[0] + DiffCoefSumOverElectronsFlux[1]);
	DLongFlux2ndmethod = DiffCoefSumOverElectronsFlux[2];

	DTransvFlux2ndmethodError = (DiffCoeff2methodErrorFlux.col(0) + DiffCoeff2methodErrorFlux.col(1)).mean()/2.0/std::sqrt(nElectrons);
	DLongFlux2ndmethodError = (DiffCoeff2methodErrorFlux.col(2)).mean()/std::sqrt(nElectrons);
