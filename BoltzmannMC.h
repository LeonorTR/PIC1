...
public:
	...
	Eigen::ArrayXXd electronBirthPositions;         			// matrix with electrons birth positions with size {nElectrons, 3} [m]
	Eigen::ArrayXXd electronDisplacements;						// matrix with electron displacements (calculated after reaching steady state) without taking into account the change in position due to renormalization {nElectrons, 3} [m]
	Eigen::ArrayXXd electronDisplacementsWithPositionChange; 	// matrix with electron displacements (calculated after reaching steady state) taking into account the change in position due to renormalization {nElectrons, 3} [m]
	...
	// variables related to the calculation of bulk and flux diffusion coefficients (1st and 2nd method)
	int ConstTauSampling = 1000; 	// integer number by which the sampling time is multiplied to obtain Tau when calculating the diffusion coefficients using the 1st and 2nd method
	int countsTau; 					// number of points used to calculate the diffusion coefficient, given ConstTauSampling and the number of Integration Points
	double TauMed; 					// Mean Tau value

	// ----------------- 1st method variables ----------------------
	//variables for the bulk and flux velocity
	Eigen::Array3d averagedDisplacementVelocity;								// array with electron average flux velocity (calculated from the average of the electron displacements without taking into account the change in position due to renormalization) [m.s-1]
	Eigen::Array3d averagedDisplacementVelocityError;							// array with the average flux velocity error
	Eigen::Array3d averagedDisplacementVelocityWithPositionChange;				// array with electron average bulk velocity (calculated from the average of the electron displacements taking into account the change in position due to renormalization) [m.s-1]
	Eigen::Array3d averagedDisplacementVelocityWithPositionChangeError;			// array with the average bulk velocity error
	Eigen::ArrayXXd previousSamplingPointElectronDisplacements;					// variable to store the value of electronDisplacement at the previous sampling point
	Eigen::ArrayXXd previousSamplingPointElectronDisplacementsWithPositionChange; // variable to store the value of electronDisplacementWithPositionChange at the previous sampling point
	Eigen::ArrayXXd electronDisplacementsSquare;								
	Eigen::ArrayXXd electronDisplacementsWithPositionChangeSquare;

	// variables for the calculation of the diffusion coefficient
	Eigen::ArrayXXd previousElectrondisplacements; 						// electronDisplacement at the previous time Tau
	Eigen::ArrayXXd previousElectrondisplacementsWithPositionChange;	// electronDisplacementWithPositionChange at the previous time Tau
	
	// variables to calculate the bulk diffusion coefficient
	Eigen::ArrayXXd squareDisplacementsXYZBulk; 				// matrix containing the square displacement in each direction taken at times Tau for each electron
	Eigen::ArrayXd squareDisplacementsZBulk; 					// array containing the square of the displacement along z taken at times Tau minus the mean z displacement
	double DTransvBulk1stmethod; 								// average over the swarm of the transverse bulk diffusion coefficient
	double DLongBulk1stmethod; 									// average over the swarm of the longitudinal bulk diffusion coefficient
	double DTransvBulk1stmethodError;
	double DLongBulk1stmethodError;
	std::vector<std::vector<double>> effectiveDisplacementZBulk; // vector (for each electron) containing vectors to store the z-displacements that occur at each consecutive time interval Tau
	Eigen::ArrayXXd DiffCoeffErrorsBulk; 						// matrix containig, in the first column, the transverse bulk diffusion coefficient error and, in the second column, the longitudinal bulk diffusion coefficient error
	Eigen::ArrayXXd SquareSquareDisplacementZBulk; 				// usefull matrix to help in calculating the flux diffusion coefficient error
	Eigen::ArrayXd z_medBulk; 									// mean of the displacement along z taken at each consecutive time interval Tau

	// variables to calculate the flux diffusion coefficient
	Eigen::ArrayXXd squareDisplacementsXYZFlux; 				// matrix containing the square displacement in each direction taken at times Tau for each electron
	Eigen::ArrayXd squareDisplacementsZFlux; 					// array containing the square of the displacement along z taken at times Tau minus the mean z displacement
	double DTransvFlux1stmethod; 								// average over the swarm of the transverse Flux diffusion coefficient
	double DLongFlux1stmethod;									// average over the swarm of the longitudinal Flux diffusion coefficient
	double DTransvFlux1stmethodError;
	double DLongFlux1stmethodError;

	std::vector<std::vector<double>> effectiveDisplacementZFlux; // vector (for each electron) containing vectors to store the z-displacements that occur at each consecutive time interval Tau
	Eigen::ArrayXXd DiffCoeffErrorsFlux; 						// matrix containig, in the first column, the transverse Flux diffusion coefficient error and, in the second column, the longitudinal Flux diffusion coefficient error
	Eigen::ArrayXXd SquareSquareDisplacementZFlux; 				// usefull matrix to help in calculating the flux diffusion coefficient error
	Eigen::ArrayXd z_medFlux; 									// mean of the displacement along z taken at each consecutive time interval Tau

	// --------------- 2nd method variables -------------------
	Eigen::ArrayXXd W_Bulk;			// Matrix containg the bulk velocity of each electron
	Eigen::ArrayXXd W_BulkError;
	Eigen::ArrayXXd SquareMeanW_Bulk; // usefull matrix to help calculating the bulk velocity

	Eigen::ArrayXXd DiffCoeff2methodBulk; 						// matrix containing the bulk diffusion coefficients Dxx, Dyy, Dzz, considering an electric field along z for each electron
	Eigen::ArrayXXd DiffCoeff2methodErrorBulk; 					// matrix containing the bulk diffusion coefficients DxxError, DyyError, DzzError, considering an electric field along z for each electron
	double DTransvBulk2ndmethod, DLongBulk2ndmethod, DTransvBulk2ndmethodError, DLongBulk2ndmethodError; // Transverse and longitudinal diffusion coefficiens and respective errors
	Eigen::ArrayXXd positionS_1Bulk;							// matrix containig the electrons position at the sampling point s-1 (the previous sampling point, taking s as the reference point)
	Eigen::ArrayXXd positionS_2Bulk;							// matrix containig the electrons position at the sampling point s-2 (taking s as the reference sampling point point)
	Eigen::ArrayXXd DeltaR2MeanElectronsBulk;					
	Eigen::ArrayXXd DeltaR1MeanElectronsBulk;
	Eigen::ArrayXXd DeltaR2DeltaR2_DeltaR1DeltaR1Bulk;
	Eigen::ArrayXXd DeltaR2DeltaR2_DeltaR1DeltaR1SquareBulk;
	Eigen::ArrayXXd DeltaR1DeltaR1Bulk;
	Eigen::ArrayXXd DeltaR2DeltaR2Bulk;

	Eigen::ArrayXXd DiffCoeff2methodFlux; 						// matrix containing the Flux diffusion coefficients Dxx, Dyy, Dzz, considering an electric field along z for each electron
	Eigen::ArrayXXd DiffCoeff2methodErrorFlux; 					// matrix containing the Flux diffusion coefficients DxxError, DyyError, DzzError, considering an electric field along z for each electron
	double DTransvFlux2ndmethod, DLongFlux2ndmethod, DTransvFlux2ndmethodError, DLongFlux2ndmethodError; // Transverse and longitudinal diffusion coefficiens and respective errors
	Eigen::ArrayXXd positionS_1Flux;							// matrix containig the electrons position at the sampling point s-1 (the previous sampling point, taking s as the reference point)
	Eigen::ArrayXXd positionS_2Flux;							// matrix containig the electrons position at the sampling point s-2 (taking s as the reference sampling point point)
	Eigen::ArrayXXd DeltaR2MeanElectronsFlux;
	Eigen::ArrayXXd DeltaR1MeanElectronsFlux;
	Eigen::ArrayXXd DeltaR2DeltaR2_DeltaR1DeltaR1Flux;
	Eigen::ArrayXXd DeltaR2DeltaR2_DeltaR1DeltaR1SquareFlux;
	Eigen::ArrayXXd DeltaR1DeltaR1Flux;
	Eigen::ArrayXXd DeltaR2DeltaR2Flux;
	...
	BoltzmannMC(SetupType* setup){
		...
		if (FieldInfo::getField("electronKinetics.numericsMC.ConstTauSampling")){
			ConstTauSampling = FieldInfo::getFieldNumericValue("electronKinetics.numericsMC.ConstTauSampling");
		}			
