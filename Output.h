...
public:
	...
	void saveSwarm(std::map<std::string,double> &swarmParam, double reducedElecField, double elecFieldAngle, double reducedMagField, double excitationFrequency){
		...
		if (eedfType == "prescribedEedf"){
			...
			std::fprintf(fileID, "\n%s\n\n", (std::string(15,'*') + " " + "Displacement parameters" + " " + std::string(15,'*')).c_str() );
			velRelErr = electronKinetics->averagedDisplacementVelocityError/Eigen::abs(electronKinetics->averagedDisplacementVelocity)*100.0;
			std::fprintf(fileID, "                                   | v_x Flux (1st method) |   | %-15.8e |                       | %-9.3e%% |\n", electronKinetics->averagedDisplacementVelocity[0], velRelErr[0]);
			std::fprintf(fileID, "                                   | v_y Flux (1st method) | = | %-15.8e |  (m/s)    ; Rel. std: | %-9.3e%% |\n", electronKinetics->averagedDisplacementVelocity[1], velRelErr[1]);
			std::fprintf(fileID, "                                   | v_z Flux (1st method) |   | %-15.8e |                       | %-9.3e%% |\n\n", electronKinetics->averagedDisplacementVelocity[2], velRelErr[2]);

			velRelErr = electronKinetics->averagedDisplacementVelocityWithPositionChangeError/Eigen::abs(electronKinetics->averagedDisplacementVelocityWithPositionChange)*100.0;
			std::fprintf(fileID, "                                   | v_x Bulk (1st method) |   | %-15.8e |                       | %-9.3e%% |\n", electronKinetics->averagedDisplacementVelocityWithPositionChange[0], velRelErr[0]);
			std::fprintf(fileID, "                                   | v_y Bulk (1st method) | = | %-15.8e |  (m/s)    ; Rel. std: | %-9.3e%% |\n", electronKinetics->averagedDisplacementVelocityWithPositionChange[1], velRelErr[1]);
			std::fprintf(fileID, "                                   | v_z Bulk (1st method) |   | %-15.8e |                       | %-9.3e%% |\n\n", electronKinetics->averagedDisplacementVelocityWithPositionChange[2], velRelErr[2]);

			// Bulk diffusion coefficient 1st method
			double TransverseBulkDiffusionCoeff = electronKinetics->DTransvBulk1stmethod*electronKinetics->totalGasDensity;
			double LongitudinalBulkDiffusionCoeff = electronKinetics->DLongBulk1stmethod*electronKinetics->totalGasDensity;
			double relaverageTransverseBulkDiffusionCoeffError = electronKinetics->DTransvBulk1stmethodError/electronKinetics->DTransvBulk1stmethod;
			double relaverageLongitudinalBulkDiffusionCoeffError = electronKinetics->DLongBulk1stmethodError/electronKinetics->DLongBulk1stmethod;
			// Flux diffusion coefficient 1st method
			double TransverseFluxDiffusionCoeff = electronKinetics->DTransvFlux1stmethod*electronKinetics->totalGasDensity;
			double LongitudinalFluxDiffusionCoeff = electronKinetics->DLongFlux1stmethod*electronKinetics->totalGasDensity;
			double relaverageTransverseFluxDiffusionCoeffError = electronKinetics->DTransvFlux1stmethodError/electronKinetics->DTransvFlux1stmethod;
			double relaverageLongitudinalFluxDiffusionCoeffError = electronKinetics->DLongFlux1stmethodError/electronKinetics->DLongFlux1stmethod;

			std::fprintf(fileID, "  Bulk Transverse diffusion coefficient (1st method) = %#.14e (1/(ms)) ; Rel. std: %-9.3e%%\n", TransverseBulkDiffusionCoeff, relaverageTransverseBulkDiffusionCoeffError*100.0);
			std::fprintf(fileID, "Bulk Longitudinal diffusion coefficient (1st method) = %#.14e (1/(ms)) ; Rel. std: %-9.3e%%\n", LongitudinalBulkDiffusionCoeff, relaverageLongitudinalBulkDiffusionCoeffError*100.0);
			std::fprintf(fileID, "  Flux Transverse diffusion coefficient (1st method) = %#.14e (1/(ms)) ; Rel. std: %-9.3e%%\n", TransverseFluxDiffusionCoeff, relaverageTransverseFluxDiffusionCoeffError*100.0);
			std::fprintf(fileID, "Flux Longitudinal diffusion coefficient (1st method) = %#.14e (1/(ms)) ; Rel. std: %-9.3e%%\n", LongitudinalFluxDiffusionCoeff, relaverageLongitudinalFluxDiffusionCoeffError*100.0);
			std::fprintf(fileID, "nCounts: %d \n", electronKinetics->countsTau);
			std::fprintf(fileID, "Mean Tau: %-15.8e \n", electronKinetics->TauMed); 
			std::fprintf(fileID, "Total Integrated Time: %-15.8e \n", electronKinetics->totalIntegratedTime);
			// Bulk diffusion coefficient 2nd method
			double TransverseBulkDiffusionCoeff2ndM = electronKinetics->DTransvBulk2ndmethod*electronKinetics->totalGasDensity;
			double LongitudinalBulkDiffusionCoeff2ndM = electronKinetics->DLongBulk2ndmethod*electronKinetics->totalGasDensity;
			double TransverseBulkDiffusionCoeff2ndMError = electronKinetics->DTransvBulk2ndmethodError/electronKinetics->DTransvBulk2ndmethod;
			double LongitudinalBulkDiffusionCoeff2ndMError = electronKinetics->DLongBulk2ndmethodError/electronKinetics->DLongBulk2ndmethod;
			// Flux diffusion coefficient 2nd method
			double TransverseFluxDiffusionCoeff2ndM = electronKinetics->DTransvFlux2ndmethod*electronKinetics->totalGasDensity;
			double LongitudinalFluxDiffusionCoeff2ndM = electronKinetics->DLongFlux2ndmethod*electronKinetics->totalGasDensity;
			double TransverseFluxDiffusionCoeff2ndMError = electronKinetics->DTransvFlux2ndmethodError/electronKinetics->DTransvFlux2ndmethod;
			double LongitudinalFluxDiffusionCoeff2ndMError = electronKinetics->DLongFlux2ndmethodError/electronKinetics->DLongFlux2ndmethod;
			Eigen::Array3d w_BulkTot = electronKinetics->W_Bulk.colwise().mean();
			Eigen::Array3d w_BulkErrorTot = electronKinetics->W_BulkError.colwise().mean()/std::sqrt(electronKinetics->nElectrons);
			
			std::fprintf(fileID, "                                   | v_x Bulk (2nd method) |   | %-15.8e |                       | %-9.3e%% |\n", w_BulkTot[0], std::abs(w_BulkErrorTot[0]/w_BulkTot[0])*100.0);
			std::fprintf(fileID, "                                   | v_y Bulk (2nd method) | = | %-15.8e |  (m/s)    ; Rel. std: | %-9.3e%% |\n", w_BulkTot[1], std::abs(w_BulkErrorTot[1]/w_BulkTot[1])*100.0);
			std::fprintf(fileID, "                                   | v_z Bulk (2nd method) |   | %-15.8e |                       | %-9.3e%% |\n\n", w_BulkTot[2], std::abs(w_BulkErrorTot[2]/w_BulkTot[2])*100.0);

			std::fprintf(fileID, "  Bulk Transverse diffusion coefficient (2nd Method) = %#.14e (1/(ms)) ; Rel. std: %-9.3e%%\n", TransverseBulkDiffusionCoeff2ndM, TransverseBulkDiffusionCoeff2ndMError*100.0);
			std::fprintf(fileID, "Bulk Longitudinal diffusion coefficient (2nd Method) = %#.14e (1/(ms)) ; Rel. std: %-9.3e%%\n", LongitudinalBulkDiffusionCoeff2ndM, LongitudinalBulkDiffusionCoeff2ndMError*100.0);

			std::fprintf(fileID, "  Flux Transverse diffusion coefficient (2nd Method) = %#.14e (1/(ms)) ; Rel. std: %-9.3e%%\n", TransverseFluxDiffusionCoeff2ndM, TransverseFluxDiffusionCoeff2ndMError*100.0);
			std::fprintf(fileID, "Flux Longitudinal diffusion coefficient (2nd Method) = %#.14e (1/(ms)) ; Rel. std: %-9.3e%%\n", LongitudinalFluxDiffusionCoeff2ndM, LongitudinalFluxDiffusionCoeff2ndMError*100.0);
	...
	void saveLookUpTable(WorkingConditions* &workCond, GeneralDefinitions::PowerStruct &power, std::map<std::string,double> &swarmParam){
		...
		if (eedfType == "prescribedEedf"){
			...
			// flux diffusion coefficient calculated using 1st method
			double TransverseBulkDiffusionCoeff = electronKinetics->DTransvFlux1stmethod*electronKinetics->totalGasDensity;
			double LongitudinalBulkDiffusionCoeff = electronKinetics->DLongFlux1stmethod*electronKinetics->totalGasDensity;
		    ...
		    std::fprintf(fileID1, "%-21.14e %-21.14e %-21.14e %-21.14e %-21.14e %-21.14e %-21.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e %-23.14e \n", conditionValue, 
		    ...
		    swarmParam["bulkRedMobCoeff"], swarmParam["fluxRedMobCoeff"],
		    swarmParam["bulkRedTransvDiffCoeff"], swarmParam["bulkRedLongDiffCoeff"], swarmParam["fluxRedTransvDiffCoeff"], swarmParam["fluxRedLongDiffCoeff"],
		    electronKinetics->averagedDisplacementVelocityWithPositionChange[0], electronKinetics->averagedDisplacementVelocityWithPositionChange[1], electronKinetics->averagedDisplacementVelocityWithPositionChange[2],
		    electronKinetics->averagedDisplacementVelocity[0], electronKinetics->averagedDisplacementVelocity[1], electronKinetics->averagedDisplacementVelocity[2], 
		    TransverseBulkDiffusionCoeff, LongitudinalBulkDiffusionCoeff);	
